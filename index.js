...
inputSearch.addEventListener("input", function () {
  const str = this.value.trim(); // Remove espaços em branco do início e fim
  if (str) {
    filterData(str);
  } else {
    showAllItems();
...
function filterData(str) {
  showAllItems(); // Resetar a exibição de todas as tarefas

  const taskItems = document.querySelectorAll('#taskList li');

  taskItems.forEach((taskItem) => {
    const taskTitle = taskItem.querySelector('h2').textContent.toLocaleUpperCase(); // Convertendo título da tarefa para maiúsculas
    const searchTerm = str.toLocaleUpperCase(); // Convertendo termo de busca para maiúsculas

    if (taskTitle.includes(searchTerm)) {
      taskItem.style.display = "block"; // Exibir a tarefa se o título contém o termo de busca
    } else {
      taskItem.style.display = "none"; // Ocultar a tarefa se não há correspondência
    }
  });
}
